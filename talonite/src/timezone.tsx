...
const ALL_ZONES_SORTED = [LOCAL_TZ, ...ALL_ZONES.filter((z) => z !== LOCAL_TZ).sort()];
...
const CITY_ALIASES: Record<string, string> = {
  nyc: "America/New_York", dc: "America/New_York", bos: "America/New_York",
  phi: "America/New_York", atl: "America/New_York", mia: "America/New_York",
  tor: "America/Toronto", yyz: "America/Toronto", chi: "America/Chicago",
  dfw: "America/Chicago", den: "America/Denver", phx: "America/Phoenix",
  la: "America/Los_Angeles", lax: "America/Los_Angeles", sf: "America/Los_Angeles",
  sfo: "America/Los_Angeles", sea: "America/Los_Angeles", pdx: "America/Los_Angeles",
  yvr: "America/Vancouver", mex: "America/Mexico_City",
  gru: "America/Sao_Paulo", sao: "America/Sao_Paulo",
  bue: "America/Argentina/Buenos_Aires", scl: "America/Santiago",
  lon: "Europe/London", ldn: "Europe/London", lhr: "Europe/London",
  dub: "Europe/Dublin", par: "Europe/Paris", ber: "Europe/Berlin",
  muc: "Europe/Berlin", rom: "Europe/Rome", mil: "Europe/Rome",
  mad: "Europe/Madrid", bc: "Europe/Madrid", lis: "Europe/Lisbon",
  ams: "Europe/Amsterdam", bru: "Europe/Brussels", zur: "Europe/Zurich",
  vie: "Europe/Vienna", bru: "Europe/Brussels", cph: "Europe/Copenhagen",
  sto: "Europe/Stockholm", osl: "Europe/Oslo", hel: "Europe/Helsinki",
  athen: "Europe/Athens", ist: "Europe/Istanbul", msq: "Europe/Moscow",
  tyo: "Asia/Tokyo", tok: "Asia/Tokyo", hnd: "Asia/Tokyo",
  sel: "Asia/Seoul", icn: "Asia/Seoul", pek: "Asia/Shanghai",
  sha: "Asia/Shanghai", hkg: "Asia/Hong_Kong", tpe: "Asia/Taipei",
  sin: "Asia/Singapore", kul: "Asia/Kuala_Lumpur", bkk: "Asia/Bangkok",
  sgn: "Asia/Ho_Chi_Minh", mnl: "Asia/Manila", jak: "Asia/Jakarta",
  del: "Asia/Kolkata", bom: "Asia/Kolkata", blr: "Asia/Kolkata",
  ktm: "Asia/Kathmandu", dxb: "Asia/Dubai", riy: "Asia/Riyadh",
  tbs: "Asia/Tbilisi",
  syd: "Australia/Sydney", mel: "Australia/Melbourne", bri: "Australia/Brisbane",
  per: "Australia/Perth", adl: "Australia/Adelaide",
  akl: "Pacific/Auckland", wlg: "Pacific/Auckland", hnl: "Pacific/Honolulu",
};
...
const COUNTRY_ZONES: Record<string, string[]> = {
  ...
  usa: ["America/New_York", "America/Chicago", "America/Denver", "America/Phoenix",
        "America/Los_Angeles", "America/Anchorage", "Pacific/Honolulu"],
  ...
  qa: ["Asia/Qatar"], eg: ["Africa/Cairo"], za: ["Africa/Johannesburg"],
  ng: ["Africa/Lagos"], ke: ["Africa/Nairobi"], ma: ["Africa/Casablanca"],
  in: ["Asia/Kolkata"], pk: ["Asia/Karachi"], bd: ["Asia/Dhaka"],
  lk: ["Asia/Colombo"], np: ["Asia/Kathmandu"],
  cn: ["Asia/Shanghai", "Asia/Urumqi"], jp: ["Asia/Tokyo"], kr: ["Asia/Seoul"],
  hk: ["Asia/Hong_Kong"], tw: ["Asia/Taipei"], sg: ["Asia/Singapore"],
  my: ["Asia/Kuala_Lumpur"], th: ["Asia/Bangkok"], vn: ["Asia/Ho_Chi_Minh"],
  ph: ["Asia/Manila"], id: ["Asia/Jakarta", "Asia/Makassar", "Asia/Jayapura"],
  ...
  br: ["America/Sao_Paulo", "America/Recife", "America/Manaus", "America/Rio_Branco"],
  ar: ["America/Argentina/Buenos_Aires"], cl: ["America/Santiago"],
  pe: ["America/Lima"], co: ["America/Bogota"], mx: ["America/Mexico_City", "America/Tijuana"],
  cu: ["America/Havana"],
};
...
export function searchZones(query: string, exclude: string[]): string[] {
  ...
  if (COUNTRY_ZONES[compact]) COUNTRY_ZONES[compact].forEach((tz, i) => push(tz, 9_000 - i));

  const cap = q.length <= 3 ? 0 : q.length <= 8 ? 1 : 2;
  for (const tz of ALL_ZONES) {
    if (exclude.includes(tz)) continue;
    const fields = [norm(tz), norm(zoneTitle(tz)),
    ...
    for (const field of fields) {
      if (!field) continue;
      if (field === q) { best = Math.max(best, 5_000); continue; }
      ...
      if (q.length >= 4) {
        ...
        if (wd <= 3) {   // per-word caps already bound each word individually
  ...
  return out.sort((a, b) => b.score - a.score).slice(0, 50).map((r) => r.tz);
}
...
function relativeLabel(tz: string, date: Date): string {
  ...
  const m = abs % 60;
  const core = h === 0 ? `${m}m` : m === 0 ? `${h}h` : `${h}:${String(m).padStart(2, "0")}h`;
...
const ASSET_OFFSETS = new Set([
  ...
  "UTC-9", "UTC-10", "UTC-11", "UTC-12", "UTC+1", "UTC+2", "UTC+3", "UTC+3.5",
  "UTC+4", "UTC+4.5", "UTC+5", "UTC+5.5", "UTC+6", "UTC+7", "UTC+8", "UTC+9",
]);

// ------------------------------------------------------------ custom time

function CustomTime({ initial, onPick }: { initial: Date; onPick: (d: Date) => void }) {
  ...
  return (
        ...
        <ActionPanel>
          <Action.SubmitForm
            ...
            onSubmit={() => {
              ...
              onPick(instant);
              void showToast({ style: Toast.Style.Success, title: `Reference set in ${zoneTitle(zone)}` });
              pop();
            }}
          />
        </ActionPanel>
    ...
    >
      <Form.DatePicker id="when" title="Time" value={wall} onChange={(d) => d && setWall(d)} />
      <Form.Dropdown id="zone" title="In timezone" value={zone} onChange={changeZone}>
        {ALL_ZONES_SORTED.map((tz) => (
          <Form.Dropdown.Item key={tz} value={tz} title={`${zoneTitle(tz)}  (${offsetString(tz, new Date())})`} />
        ))}
      </Form.Dropdown>
...
function Reorder({ zones, onSave }: { zones: string[]; onSave: (z: string[]) => void }) {
  const { pop } = useNavigation();
              ...
              <Action
                ...
                onAction={() =>
                  confirmAlert({
                    ...
                    primaryAction: { title: "Remove", style: Alert.ActionStyle.Destructive, onAction: () => remove(i) },
                  })
                }
              />
            </ActionPanel>
...
function PinSearch({ pinned, onPin }: { pinned: string[]; onPin: (tz: string) => void }) {
  const { pop } = useNavigation();
  ...
  return (
    ...
    >
      {results.length === 0 && <List.EmptyView title="No matches" description="Try a city code, country code, or part of a name" />}
      {results.map((tz) => {
        const alreadyPinned = pinned.includes(tz);
        return (
          <List.Item
            key={tz}
            icon={{ source: alreadyPinned ? Icon.CheckCircle : Icon.Circle,
                    tintColor: alreadyPinned ? "#CA8A04" : "#99947F" }}
            title={zoneTitle(tz)}
            subtitle={offsetString(tz, at)}
            accessories={[{ text: regionName(tz) }, alreadyPinned ? { tag: { value: "pinned", color: "#CA8A04" } } : { tag: timeIn(tz, at) }]}
            actions={
              <ActionPanel>
                {alreadyPinned ? (
                  <Action title={`Unpin ${zoneTitle(tz)}`} icon={Icon.Minus} onAction={() => onPin(tz)} />
                ) : (
                  <Action
...
export default function Command() {
  ...
  function toggle(tz: string) {
    setZones(zones.includes(tz) ? zones.filter((z) => z !== tz) : [...zones, tz]);
  }
    ...
    return (
      <ActionPanel>
        ...
        <Action
          ...
          icon={Icon.Clock}
          shortcut={isCustom ? { modifiers: ["cmd"], key: "0" } : { modifiers: ["cmd"], key: "t" }}
  ...
  if (!preferences.hideWorldMap) {
    ...
    return (
      <Detail
        ...
        metadata={
          <Detail.Metadata>
            {zones.length === 0 && <Detail.Metadata.Label title="No timezones pinned" text="⌘P to pin one" />}
            {rows.map((r) => (
              <Detail.Metadata.TagList key={r.tz} title={`${zoneTitle(r.tz)} (${r.offset})`}>
                <Detail.Metadata.TagList.Item text={`${r.time}${r.shift !== 0 ? `, ${r.date}` : ""}`} />
  ...
  return (
    <List navigationTitle={isCustom ? `Ref: ${at.toLocaleString()}` : "Timezones"} actions={pinActions()}>
      {rows.length === 0 && <List.EmptyView title="No timezones pinned" description="⌘P to pin one" />}
      {rows.map((r) => (
        <List.Item
          key={r.tz}
          icon={{
            source: r.shift === 0 ? Icon.Circle : r.shift === 1 ? Icon.ArrowUp : Icon.ArrowDown,
            tintColor: r.shift === 0 ? "#99947F" : r.shift === 1 ? "#CA8A04" : "#3D82F2",
          }}
          ...
          actions={
            <ActionPanel>
              ...
              />
              <Action title="Unpin" icon={Icon.Minus} onAction={() => toggle(r.tz)} />
              {pinActions()}
            </ActionPanel>
          }
        />
